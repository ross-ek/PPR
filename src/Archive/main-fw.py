...
'''
this version splits coutnies into different data frames
'''
...
def simple_addr(s):
    ...
    x = re.sub('^APART ,|^APT ,|^APPT ,|^APPAR ','', x)
...
def match_by_county(df_addr, df_eircode, county):
    ...
    if county == 'LEITRIM':
        ...
        df_addr['Town2'] = df_addr['Town'].apply(lambda x: process.extractOne(x, df_eircode['name']))
...
def main():
    ...
    df_ppr.to_csv(dir_output + 'output_ppr-' + format(dt.datetime.now().strftime("%Y%m%d-%H%M")) +'.csv', index=False)

    for i in county_towns:
        df_temp = match_by_county(df_ppr,df_eirc,i)
        df_out = df_out.append(df_temp) 

    df_out.to_csv(dir_output + 'output_combined-' + format(dt.datetime.now().strftime("%Y%m%d-%H%M")) +'.csv', index=False)
